...
class JugadorTest extends munit.FunSuite {
  ...
  override def beforeEach(context: BeforeEach): Unit = {
    Jugador1 = Jugador("Jugador 1",1,2,25,0)
    Jugador2 = Jugador("Jugador 2",2,2,25,0)
  ...
  test("El jugador tiene una cantidad de gemas"){
    ...
    assert(Jugador1.gemas > (new Jugador("J5",5,1,25,0)).gemas,"1 tiene menos gemas que el jugador nuevo") // las gemas de 1 son mas que las de 2
  }
  test("Jugador tiene un mazo de cartas"){
    ...
    Jugador2.tomar_cartas_mazo(3)
    Jugador2.tomar_cartas_mazo(2)
    assertEquals(Jugador2.mazo, expected = 10)  // El J2 saca 5 cartas del mazo, quedando 10
    Jugador3.tomar_cartas_mazo(2)
  ...
  test("Un jugador tiene todos sus atributos definidos"){
    assertEquals(new Jugador("Jugador4",4,2,25,0),new Jugador("Jugador4",4,2,25,0)) 
